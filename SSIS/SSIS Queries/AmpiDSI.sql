--AMPI DSI File Format
-- Remove previous data.
TRUNCATE TABLE dbo.AmpiData
...
WHERE [dbo].[AmpiCustomerCodes](CustomerCode) = '1'
	AND RowStatus = 0
	AND CommercialOrderLineStatus != 'Cancelled'

--select * from #TempAmpiData1
---------------------------------------------------------------------------------------------------------------------------------------------
SELECT Company,
	Division,
	ProducerCode,
...
SELECT dbo.PadRight2(Company) Company,
	...
	'' Fixed7,
	Seq,
	[dbo].[AmpiLoadID](CustomerFramework, Barcode) LoadId,
...
--select * into dbo.AmpiData from #TempAmpiData4 -- Use this if the table is deleted.
INSERT INTO dbo.AmpiData
SELECT *
FROM #TempAmpiData4

DROP TABLE #TempAmpiData1

DROP TABLE #TempAmpiData2

DROP TABLE #TempAmpiData3

DROP TABLE #TempAmpiData4

--select ?=count(1) from dbo.AmpiData   
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT *
FROM dbo.AmpiData
