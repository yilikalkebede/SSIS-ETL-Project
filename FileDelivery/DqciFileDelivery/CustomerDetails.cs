...
namespace DqciFileDelivery
{
    public class CustomerDetails
    {
        ...
        public bool OverRidePreFTPChecks { get; set; }
        public string CustomerSpecificFileName { get; set; }
    }
    ...
    public class CustomerSendFiles
    {
        ...
        public List<CustomerDetails> GetCustomerDetailses(List<CustomerFiles> customersFiles, string connectionString)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    command.CommandText =
                        @"CREATE TABLE #CusomersFiles (CustomerCode nvarchar(250), FilePath nvarchar(2500))";
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            ...
                            int OverRidePreFTPChecksOrdinal = reader.GetOrdinal("OverRidePreFTPChecks");
                            ...
                            while (reader.Read())
                            {
                                listOfCustomerDetails.Add(new CustomerDetails
                                {
                                    ...
                                    IsFullName = reader.GetBoolean(IsFullNameOrdinal),
                                    OverRidePreFTPChecks = reader.SafeGetBoolean(OverRidePreFTPChecksOrdinal)
                                });
